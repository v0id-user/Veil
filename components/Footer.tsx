    ...
    return (
        <div className="flex flex-col gap-3 w-fit">
            ...
            <ul className="flex flex-col gap-2">
                {links.map(({ name, url }) => (
                    <li key={name} className="hover:underline text-sm cursor-pointer transition-all duration-200 opacity-80 hover:opacity-100">
                        <a href={url}>{name}</a>
                    </li>
                ))}
            </ul>
            ...
            <div className="container mx-auto px-6 w-full">
                <div className="flex flex-col md:flex-row gap-12 justify-between items-center md:items-start">
                    ...
                    <div className="flex flex-col sm:flex-row gap-8 sm:gap-16 justify-between items-center sm:items-start">
                        ...
                        <LinksColumn title="تطبيقات" links={[{ name: "الويب", url: "/web" }, { name: "أجهزة الأندرويد", url: "/android" }, { name: "أجهزة الأي أو إس", url: "/ios" }, { name: "سطح المكتب", url: "/desktop" }]} />
                        <LinksColumn title="المساعدة" links={[{ name: "فتح تذكرة", url: "/support/ticket" }, { name: "الاستفسارات", url: "/support/inquiries" }, { name: "المساعدة", url: "/support" }]} />
                        <LinksColumn title="من التواصل" links={[{ name: "البريد الإلكتروني", url: "/contact/email" }, { name: "إكس", url: "/contact/x" }]} />
                    </div>
                </div>
            </div>
        </footer>
